...
async function prepareOffline() {
  ...
  try {
    const setup = async () => {
      ...
      const cache = await caches.open('ford-base-finder-v3');
      const required = await Promise.all(['/index.html', '/app.js', '/core.js', '/reference.txt', '/epc-reference.json'].map(path => cache.match(path)));
    ...
    offline.textContent = 'The included reference is saved for offline use on this browser. Online access may still be needed to sign in.';
  } catch {
